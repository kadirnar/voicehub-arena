...
def publish_artifact(api,identifier,phase,manifest='configs/native-methods.json'):
    import requests
    cfg,spec,selected,directory,contract=load_experiment(manifest,identifier,phase)
...
def publish_progress(api,manifest='configs/native-methods.json'):
    cfg=json.loads(Path(manifest).read_text());run=Path('runs')/cfg['campaign']
    ...
    progress=dict(campaign=cfg['campaign'],updated_at=time.time(),scope=cfg['scope'],
        ...
        controller={k:state.get(k) for k in ('status','experiment','phase','action','updated_at','active_jobs','execution_mode')})
    for spec in cfg['experiments']:
        ...
        for phase in ('pilot','full'):
            ...
            active=state.get('status')=='running' and ((state.get('experiment')==spec['id'] and state.get('phase')==phase)
                or any(j.get('experiment')==spec['id'] and j.get('phase')==phase for j in state.get('active_jobs',[])))
            worker_status=r.get('status','')
            if not active and (worker_status in ('loading','generating') or worker_status.startswith('scoring_')):
                entry[phase].update(status='paused_by_user' if state.get('status')=='stopped_by_user' else 'queued',last_worker_status=worker_status)
    ...
    fields=['experiment','family','method','streaming','generation_device','repo','phase','evaluated','generated','generation_failures',
            ...
            'rtf','latency_p50_s','latency_p95_s','ttfa_p50_s','timing_scopes','peak_vram_mib','silence_ratio','clipping_ratio','rms_dbfs',
    ...
    dataset_files={f'experiments/{cfg["campaign"]}/progress.json':path,
        ...
        f'experiments/{cfg["campaign"]}/inventory.json':Path(manifest)}
...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--experiment');p.add_argument('--phase',default='pilot',choices=['pilot','full']);p.add_argument('--manifest',default='configs/native-methods.json');a=p.parse_args()
    os.environ['HF_HOME']=str(Path.cwd()/'.cache/huggingface')
    from huggingface_hub import HfApi
    api=HfApi();assert api.whoami()['name']=='kadirnar'
    if a.experiment:publish_artifact(api,a.experiment,a.phase,a.manifest)
    publish_progress(api,a.manifest)
